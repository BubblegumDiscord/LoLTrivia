...
from cassiopeia.type.core.common import Region, Platform
...
def make_request(request, method, params={}, payload=None, static=False, include_base=True, tournament=False):
    ...
    # Set server and rgn
    server = Platform[Region(region).name].value.lower()
    rgn = ("static-data" if static else "{region}").format(region=region)
    ...
    if include_base:
        url = "https://{server}.api.riotgames.com/lol/{region}/{request}?{params}".format(server=server, region=rgn, request=request, params=encoded_params)
    else:
